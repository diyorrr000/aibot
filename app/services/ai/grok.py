...
GROK_API_URL = "http://de3.bot-hosting.net:21007/kilwa-grok"
    ...
    async def generate_response(
        ...
        for attempt in range(retries):
            try:
                ...
                last_error = e
                logger.warning(f"Grok API attempt {attempt+1}/{retries} failed: {e}")
                await asyncio.sleep(delay)
                delay *= 1.5

        raise Exception(f"Grok API error: {last_error}")
