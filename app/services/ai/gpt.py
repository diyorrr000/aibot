...
GPT_API_URL = "http://de3.bot-hosting.net:21007/kilwa-chatgpt"
    ...
    async def generate_response(
        ...
        for attempt in range(retries):
            try:
                ...
                last_error = e
                logger.warning(f"GPT API attempt {attempt+1}/{retries} failed: {e}")
                await asyncio.sleep(delay)
                delay *= 1.5

        raise Exception(f"GPT API error: {last_error}")
