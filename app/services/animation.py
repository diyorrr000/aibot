...
async def run_aiogram_animation(bot: Bot, chat_id: int, anim_name: str, conn_id: str = None):
    ...
    try:
        msg = await bot.send_message(chat_id=chat_id, text=frames[0], business_connection_id=conn_id, parse_mode='HTML')
    except Exception:
        use_conn = None
        try:
            msg = await bot.send_message(chat_id=chat_id, text=frames[0], parse_mode='HTML')
        except Exception:
            return
